...
The UI consumes only snapshots conforming to `contracts/provider-snapshot.schema.json`. It does not parse Codex responses or infer missing provider fields.
...
The Codex adapter is a short-lived, standard-library helper that exchanges bounded JSON-RPC messages with `codex app-server`, emits one normalized snapshot, and exits. It has a whole-refresh deadline, a total output cap, fixed process arguments, static failure messages, and deterministic cleanup. This avoids a second long-lived process between refreshes.

QML consumes only helper stdout and never logs provider output or stderr. A malformed snapshot becomes a static `ERROR` state. Synthetic multi-provider data can be selected only through `OMARCHY_AI_USAGE_PREVIEW=1`, and its expanded card visibly labels that mode. Normal operation renders only provider snapshots backed by real adapters.
