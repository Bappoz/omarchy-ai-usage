...
QtObject {
  ...
  readonly property string helperPath: String(
    Qt.resolvedUrl("../../helpers/codex_provider.py")
  ).replace(/^file:\/\//, "")
  readonly property string fixturePath: String(
    Qt.resolvedUrl("../fixtures/compact-active.json")
  ).replace(/^file:\/\//, "")

  // Preview mode is deliberately explicit and visibly labeled by the UI.
  // Normal service startup always uses the live Codex provider.
  ...
  function loadingSnapshot() {
    return emptySnapshot("LOADING", "Reading Codex limits…")
  }

  function failedSnapshot() {
    return emptySnapshot("ERROR", "Codex usage could not be refreshed.")
  }
  ...
  function isSnapshot(value) {
    ...
    if (value.schemaVersion !== 1 || String(value.providerId || "") !== "codex") return false
    if (typeof value.displayName !== "string" || value.displayName.length === 0) return false

    var states = [
      "ACTIVE", "LOADING", "STALE", "NEEDS_AUTH",
      "RATE_LIMITED", "ERROR", "UNAVAILABLE"
    ]
    ...
    if (headline !== null && !ids[String(headline)]) return false
    return true
  }
  ...
  function applyText(text) {
    try {
      ...
      // Provider output is intentionally not logged: it may originate outside
      // this trust boundary. The UI receives a static, user-safe error only.
  ...
  function refresh() {
    ...
    if (!root.providerEnabled) {
      pollTimer.stop()
      root.snapshot = emptySnapshot("UNAVAILABLE", "Codex is disabled in preferences.")
      root.loading = false
      return
    }
  ...
  function scheduleNext() {
    if (root.previewMode || !root.providerEnabled) return
    ...
    if (status === "ERROR" || status === "UNAVAILABLE" || status === "STALE") {
      ...
      seconds = Math.min(seconds, 30 * Math.pow(2, root.consecutiveFailures - 1))
  ...
  onProviderEnabledChanged: {
    if (providerEnabled) refresh()
    else {
      pollTimer.stop()
      snapshot = emptySnapshot("UNAVAILABLE", "Codex is disabled in preferences.")
      loading = false
    }
  }

  onPollingIntervalSecondsChanged: {
    if (!loading && providerEnabled) scheduleNext()
  }
  ...
  property Process providerProcess: Process {
    ...
    // Consume stderr without displaying or logging it. Provider states arrive
    // through the normalized JSON contract on stdout.
  ...
  property FileView previewFile: FileView {
    path: root.fixturePath
    watchChanges: false
    printErrors: false

    onLoaded: {
      if (root.previewMode) root.applyText(text())
    }
